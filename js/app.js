...
function sv(k,d){localStorage.setItem(SK[k],JSON.stringify(d));syncMsg();}
function ldOne(k){try{return JSON.parse(localStorage.getItem(SK[k]))||null;}catch{return null;}}
function svOne(k,d){localStorage.setItem(SK[k],JSON.stringify(d));syncMsg();}
function syncMsg(){const e=document.getElementById('syncInfo');if(e){const t=new Date();e.innerHTML='● Saved '+t.toLocaleTimeString('en-GB',{hour:'2-digit',minute:'2-digit'});e.style.color='var(--green2)';}}
